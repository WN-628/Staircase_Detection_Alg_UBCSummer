...
def get_mixed_layers(p,ct,sa,c1,c2,c3,c4):
  ...
  max_count = int(2000 / FIXED_RESOLUTION_DBAR)  # 2000 dbar resolution
  ...
  T_av2          = moving_average2d(ct,c3)      #c3
  S_av2          = moving_average2d(sa,c3)      #c3
  ...
  for k in range(len(ml_count_tmp)):
    if ml_count_tmp[k]:
      for l in range(np.min([max_count,ml_count_tmp[k]])):
        ...
        pmin[k,l]       = int(layer.min())
        pmax[k,l]       = int(layer.max())
